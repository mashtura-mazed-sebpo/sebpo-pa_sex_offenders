...
UPS_Offenders script = new UPS_Offenders(context)
script.initParsing()
/*************Debug*******************/
...
/**SCRIPT's initializations**/
    ...
    final String mainUrl = "https://www.pameganslaw.state.pa.us/Search/SVP_SVDC_Search"
    def CACHED_DATA = []
    ...
    UPS_Offenders(context, threadsCount = 0) {
    ...
    def parseData(def html) {
        String urls = new File("PA_SEX_OFF_Updated_URLS_.txt").text
        //String urls = urlsFile.text
        detailsLinks = urls.split(/\s/)
...
//pagination
            ...
            def lastPage = driver.findElement(new By.ByXPath("/html/body/div[4]/form/div[2]/div/div[1]/div/ul/li[9]/a"))
                .getAttribute("href")
                .toString()
                .replaceAll(/.*?page=(\d+).*/, '$1')
            ...
            for (int i = 1; i <= lastPage; i++) {
                //&selectedSortBy=1
                url = "https://www.pameganslaw.state.pa.us/Search/SVP_SVDC_Search?page=$i&selectedSortBy=1"
            ...
            println("====CAPTURING ENTITY URLS COMPLETE===\n")
            driver.close()
...
//------------------------------Misc utils part---------------------//
//    def invoke(url, miscData = [:], cache = false, tidy = true, headersMap = [:]) {
//        // Thread.sleep(3000)
////        headersMap['Cookie'] = 'ASP.NET_SessionId=sjft21dpaugatzp0usgoveuj; __RequestVerificationToken=jd1emFjmCDU0PzUR8rsy7EKUIVOL3fP7dhUpHfURqiQKJPmcActo1xbB0Puttisq_Pb_xc2fHcIi4dGRXkkE8Otz8b2LfcfhZj5jaSJ1RRs1'
//        /*  if(url=~/(?i)countysearch$/){
//              headersMap['Cookie']="ASP.NET_SessionId=iokptd2y4vfiinstc0fhnxyr; __RequestVerificationToken=bA1lEmOdEnSqhln87hT1LVbHi4M3nxbsr7Xzw8UrysLe6VUyKwSoaeqBwsyFNx0kQqkA-6NnxYsuR8izA4SDrcdQCkAbRa7omgjOUr10mTI1"
//          }else if(url=~/(?i)CountyIncarcerated=true$/){
//              headersMap['Cookie'] = "ASP.NET_SessionId=iokptd2y4vfiinstc0fhnxyr; __RequestVerificationToken=pUJ8BO7esqZ37tx-2Ggg5EysBfhExGg8wyvhQR-8UFFOiRFYnHd3VFtsYhM0XIpiiIHZlR5E-5lQOapBZhbcflpB6QEZ2NpeSbDd4fPvQKI1"
//          }else if(url=~/\d+$/){
//              headersMap['Cookie']="bA1lEmOdEnSqhln87hT1LVbHi4M3nxbsr7Xzw8UrysLe6VUyKwSoaeqBwsyFNx0kQqkA-6NnxYsuR8izA4SDrcdQCkAbRa7omgjOUr10mTI1"
//          }else if(url=~/(?i)/){
//              headersMap['Cookie']="ASP.NET_SessionId=iokptd2y4vfiinstc0fhnxyr; __RequestVerificationToken=bA1lEmOdEnSqhln87hT1LVbHi4M3nxbsr7Xzw8UrysLe6VUyKwSoaeqBwsyFNx0kQqkA-6NnxYsuR8izA4SDrcdQCkAbRa7omgjOUr10mTI1"
//          }*/
//
////        def warningPage = context.invoke([url:this.url])
////        def headerToken = warningPage=~/(?ism)<input name="__RequestVerificationToken".*?value="(.*?)"/
////        if(headerToken.find()){
////            headersMap['Cookie'] = 'ASP.NET_SessionId=sjft21dpaugatzp0usgoveuj; __RequestVerificationToken='+headerToken.group(1)
////        }
//        headersMap['Cookie'] = 'ASP.NET_SessionId=sjft21dpaugatzp0usgoveuj;'
////        headersMap['Cookie'] = 'ASP.NET_SessionId=ux0yl55y1mtj23cbme1rax33;'
//        Map attrMap = [url: url, tidy: tidy, headers: headersMap, cache: cache]
//        attrMap.putAll(miscData)
//
//        try {
...
//
//        def realCookie, realCookie2 = ""
//
//        // iterate HttpCookie object
//        for (HttpCookie cookie : cookieList) {
//            if(cookie.getName() == "ASP.NET_SessionId"){
//                realCookie = cookie.toString() + ";"
//            }
//            if(cookie.getName() == "__RequestVerificationToken"){
//                realCookie2 = cookie.toString()
//            }
//        }
////        realCookie = realCookie+" "+realCookie2
//        return realCookie
//    }
...
//    final Tasker tasker
//    final String root = "https://www.pameganslaw.state.pa.us"
//    final String wurl = root + "/TermsandCondition/TermsAccepted"
//    final String url = "https://www.pameganslaw.state.pa.us/Search/NameSearch"
//    //final fl = FileLogger.getLogger("addr_check_us_pa")
//    final ConcurrentHashMap entityIdSet = new ConcurrentHashMap(100, 0.75, 7)
...
//
//    //Set the header cookie & session id
//    def cookies = getCookie(url)
...
//    private enum FIELDS
//    {
//        EYE_COLOR, GENDER, HAIR_COLOR, HEIGHT, RACE, SCARS, URL, WEIGHT, DOB
//    }
//
//    def debug(srcUrl) {
//        //def html = invoke(url, false)
//        handleDetailsPage(srcUrl)
//    }
//
////------------------------------Initial part----------------------//
//
//    def getParam(def firstName){
//        def param = [:]
//        param["enteredFirstName"]=firstName
//        param["enteredLastName"]=""
//        param["chkNameIncarcerated"]=true
//        param["chkNameIncarcerated"]=false
//        param["chkNameSoundex"]=false
//        return param
//    }
//    def getParam2(def i, def firstName){
//
//        def param =[:]
//        param["page"]=i
//        param["enteredFirstName"]=firstName
...
//    def parsingByName() {
//        def nameSearchUrl
//        def offenderSelectUrl
//        def html = invoke(url)
//
////        def dropDownMatch = html =~ /(?is)Megan's\s*Law\s*Searches(.*?)<\/ul>/
//        def dropDownMatch = html =~ /(?ism)>Offender Search(.*?)<\/ul>/
//        if (dropDownMatch) {
//            def urlMatch = dropDownMatch[0][1].toString() =~ /(?ism)<li><a[^>]*?href="([^"]*)">Name/
//            if (urlMatch) {
//                nameSearchUrl = root + urlMatch[0][1].toString()
//            }
//        }
////        html = invoke(url)
//        html = invoke(nameSearchUrl)
//        def nameResultMatch = html =~ /action="([^"]*)"\s*id="nameSearchForm"/
//        if (nameResultMatch) {
//            nameSearchUrl = root + nameResultMatch[0][1].toString().trim()
//        }
//
//        for (String alphabet = "A"; alphabet <= "B"; alphabet++) {
//            def alphabetSearchUrl = nameSearchUrl + "?enteredFirstName=$alphabet&enteredLastName=&chkNameIncarcerated=true&chkNameIncarcerated=false&chkNameSoundex=false"
//            //def alphabetSearchUrl = nameSearchUrl + "?enteredFirstName=$alphabet&enteredLastName=&chkNameIncarcerated=true&chkNameSoundex=false"
//            def headersMap=[:]
////            html = invoke(url)
//            html = invoke(alphabetSearchUrl, getParam(alphabet))
//
//            def offenderUrlMatch = html =~ /class="bodyLink"\s*href="([^"]*)">View Details/
//            while (offenderUrlMatch.find()) {
//                offenderSelectUrl = root + offenderUrlMatch.group(1).toString()
//
//                def idMatch = offenderSelectUrl =~ /\/(\d+)$/
//                def id
//                if (idMatch) {
//                    id = idMatch[0][1].toString()
//                }
//
//                if (!entityIdSet.putIfAbsent(id, 1)) {
//                    handleDetailsPage(offenderSelectUrl)
//                }
//            }
//
//            def maxPageMatch = html =~ /class="PagedList-skipToLast">.*?page=(\d+)/
//            int maxPageNumber = 0;
//            if (maxPageMatch) {
//                maxPageNumber = Integer.parseInt(maxPageMatch[0][1].toString())
//            }
//
//            if (maxPageNumber == 0) {
//                maxPageMatch = html =~ /(?s)(\d+)<\/a><\/li>\s*<li\s*class="PagedList-skipToNext">/
//                if (maxPageMatch) {
//                    maxPageNumber = Integer.parseInt(maxPageMatch[0][1].toString())
//                }
//            }
//
//            //Iterating through every page for a county
//            def i = 2;
//            while (i <= maxPageNumber) {
//                def pageUrl = "https://www.pameganslaw.state.pa.us/Search/NameSearchResults?page=$i&enteredFirstName=$alphabet&chkNameIncarcerated=True&chkNameSoundex=False"
//                //  def pageUrl = alphabetSearchUrl.replaceAll(/(enteredFirstName=)/, 'page='+i+'&$1')
//                def header2 = [:]
//                //  header2["Cookie"]="ASP.NET_SessionId=iokptd2y4vfiinstc0fhnxyr; __RequestVerificationToken=3_1k-L6nsbW751UBXZKdkUdf3XeqeBex0KuRu9To143RFwuxZjo1KbCcs6fNqfIhJ2dPMsw479hvnaaBrlB7R4XqbibsDKUaluS0GxS7eRY1"
//                //html = invoke(url)
//                try{
//                    html = invoke(pageUrl, getParam2(i,alphabet))
//                }catch(Exception e){
//
//                }
//                def j =0
//                offenderUrlMatch = html =~ /class="bodyLink"\s*href="([^"]*)">View Details/
//                while (offenderUrlMatch.find()) {
//                    final def offenderUrl = root + offenderUrlMatch.group(1).toString()
//
//                    def idMatch = offenderUrl =~ /\/(\d+)$/
//                    def id
//                    if (idMatch) {
//                        id = idMatch[0][1].toString()
//                    }
...
//                    if (!entityIdSet.putIfAbsent(id, 1)) {
//
//                        tasker.execute({handleDetailsPage(offenderUrl)}, taskRetryCount)
//                        j++
//                    }
//                }
//                //println ("Printing number of data for  pageurl [$pageUrl]: "+j)
//                i++
//            }
//        }
...
////        def dropDownMatch = html =~ /(?is)Megan's\s*Law\s*Searches(.*?)<\/ul>/
//        def dropDownMatch = html =~ /(?ism)>Offender Search(.*?)<\/ul>/
//        //setting options to search offenders by county
//        if (dropDownMatch) {
////            def urlMatch = dropDownMatch[0][1].toString() =~ /<li><a href="(.*?)">County/
//            def urlMatch = dropDownMatch[0][1].toString() =~ /(?ism)<li><a[^>]*?href="([^"]*)">County/
//            if (urlMatch) {
//                countySearchUrl = root + urlMatch[0][1].toString()
//            }
//        }
//
//        html = invoke(countySearchUrl)
//        //  html = invoke(url)
//
//        def countyListMatch = html =~ /class="county".*?href="([^"]*)"/
//
//        //selecting county's one by one from county list dropdown menu
//        while (countyListMatch.find()) {
//            countySelectUrl = root + countyListMatch.group(1).toString().replaceAll(/&amp;/, "&")
//
//            //Including Incarcenated offenders
//            countySelectUrl = countySelectUrl.replaceAll(/chkCountyIncarcerated=false/, "chkCountyIncarcerated=true")
//            html = invoke(url)
//            html = invoke(countySelectUrl)
//            def offenderUrlMatch = html =~ /class="bodyLink"\s*href="([^"]*)">View Details/
//
//            //Invoking offender details from the first page
//            while (offenderUrlMatch.find()) {
//                offenderSelectUrl = root + offenderUrlMatch.group(1).toString()
//
//                def idMatch = offenderSelectUrl =~ /\/(\d+)$/
...
//            }
//
//            //extracting maxPageNumber from the first search result page of every county
//            def maxPageMatch = html =~ /class="PagedList-skipToLast">.*?page=(\d+)/
//            int maxPageNumber = 0;
//            if (maxPageMatch) {
//                maxPageNumber = Integer.parseInt(maxPageMatch[0][1].toString())
...
//            def i = 2;
//            while (i <= maxPageNumber) {
//                def pageUrl = countySelectUrl.replaceAll(/\?selectedCounty/, "?page=$i&selectedCounty")
//                html = invoke(url)
//                html = invoke(pageUrl)
//                offenderUrlMatch = html =~ /class="bodyLink"\s*href="([^"]*)">View Details/
//                while (offenderUrlMatch.find()) {
