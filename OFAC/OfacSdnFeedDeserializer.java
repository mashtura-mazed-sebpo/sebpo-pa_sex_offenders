...
public class OfacSdnFeedDeserializer extends Deserializer {
    ...
    public Cache deserialize(Reader reader) throws Exception {
        ...
        URL tempUrl = new URL("C:\\Users\\Administrator\\Documents\\RDC_WEBSCRAPPER-SELENIUM\\development\\src\\main\\groovy\\current_scripts\\ofac_advanced_sub_process.groovy");//null;
      /*  LI_Scripts script = null;
        boolean useResolver = false;
        if(configuration ==null) {
            configuration = (Configuration) SpringManager.getInstance().getBeanFactory().getBean("configuration");//new ScrapianResolver();
            me = configuration.getBoolean("com.rdc.importer.me", false);
        }
        if(me){
            useResolver = true;
